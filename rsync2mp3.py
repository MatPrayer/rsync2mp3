#!/usr/bin/env python3
"""
rsync2mp3 — mirror a lossless music library into MP3, incrementally.
...
Lossless sources (flac, wav, alac, ape, wv, ...) are transcoded to MP3 with
LAME. Already-lossy sources (mp3, aac, opus, vorbis) are copied verbatim —
they are never re-encoded. Cover art, lyrics and playlists come along for the
ride.

MP3 exists for players that accept nothing else: car head units, old phones,
DAPs. It is the compatibility target, not the quality one — if the device can
read Opus, use rsync2opus instead and spend half the bitrate.
...
FAILURE_LOG = ".rsync2mp3-failures.log"
TMP_SUFFIX = ".r2m-tmp"
...
def config_search_path(explicit: str | None) -> list[Path]:
    ...
    if xdg:
        paths.append(Path(xdg).expanduser() / "rsync2mp3" / "config.toml")
    if sys.platform == "darwin":
        ...
        paths.append(Path.home() / "Library" / "Application Support"
                     / "rsync2mp3" / "config.toml")
    if not xdg:
        paths.append(Path("~/.config").expanduser() / "rsync2mp3" / "config.toml")
    return paths
    ...
    def _control_opts(cls) -> list[str]:
        with cls._control_lock:
            if cls._control_dir is None:
                ...
                cls._control_dir = tempfile.mkdtemp(prefix="r2mp3-ssh-", dir=base)
    ...
    def listing(self) -> Listing:
        ...
        if r.returncode != 0:
            raise Fatal(
                ...
                f"{r.stderr.decode(errors='replace').strip()}\n"
                "(rsync2mp3 needs find, and stat on a BSD remote)"
            )
        out = Listing()
...
def resolve_collisions(jobs: list[Job]) -> list[Job]:
    ...
    Unlike the Opus target, MP3 is both an output format and a common input
    one, so a directory holding `track.flac` beside `track.mp3` produces two
    jobs writing the same destination — they would overwrite each other in
...
def plan(src_listing: dict[str, Entry], source: Endpoint,
    ...
    for rel, e in sorted(src_listing.items()):
        ...
        if ext in TRANSCODE_EXT:
            jobs.append(Job(rel, with_suffix(rel, ".mp3"), "transcode", e.size, e.mtime))
    ...
    for rel, e in ambiguous:
        mp3_rel = with_suffix(rel, ".mp3")
        if not force and fresh(e.mtime, getattr(dst_listing.get(mp3_rel), "mtime", None)):
            jobs.append(Job(rel, mp3_rel, "transcode", e.size, e.mtime))
            continue
        if not force and fresh(e.mtime, getattr(dst_listing.get(rel), "mtime", None)):
            jobs.append(Job(rel, rel, "copy", e.size, e.mtime))
            continue
        ...
        jobs.append(Job(rel, mp3_rel if lossless else rel,
...
def extract_embedded_cover(url: str, tmpdir: Path) -> tuple[bytes, str, int, int] | None:
    info = probe(url)
    if not info:
        return None
    for s in info.get("streams", ()):
...
def embed_cover(mp3_path: Path, cover: tuple[bytes, str, int, int]) -> None:
    """Write cover art into an MP3 as an ID3v2 APIC frame.
    ...
    # One front cover, no leftovers: some rips carry half a dozen APIC frames
    # and players pick between them unpredictably.
    tags.delall("APIC")
    tags.add(APIC(encoding=0, mime=sniff_mime(data, mime), type=3,
                  desc="Cover", data=data))
    # v2.3 rather than the default 2.4 — car stereos and older DAPs routinely
    # ignore 2.4 tags altogether.
...
class Context:
    ...
    def folder_cover(self, rel: str) -> tuple[bytes, str, int, int] | None:
        ...
        if cover_rel:
            try:
                with tempfile.TemporaryDirectory(prefix="r2m-art-") as td:
...
# MP3 is a narrow format: 8-48 kHz only, one or two channels only, and a fixed
# set of sample formats. A 96 kHz 24-bit surround master is perfectly ordinary
# in a lossless library and libmp3lame refuses all three of its properties, so
# everything is normalised on the way in. aformat inserts the resampler and the
# downmix itself, which is why there is no surround retry path here.
LAME_RATES = "48000|44100|32000|24000|22050|16000|12000|11025|8000"
LAME_FILTER = (f"aformat=sample_fmts=fltp|s32p|s16p"
               f":sample_rates={LAME_RATES}"
               f":channel_layouts=stereo|mono")


def encode_cmd(url: str, out: Path, bitrate: str, vbr: int | None = None,
               compression_level: int = 0) -> list[str]:
    # compression_level is LAME's -q: search effort, not bitrate. 0 is the
    # slowest and most careful, and mp3 encoding is fast enough that paying for
    # it is the obvious default.
    cmd = ["ffmpeg", "-v", "error", "-y", "-i", url, "-map", "0:a:0", "-vn",
           "-af", LAME_FILTER, "-c:a", "libmp3lame",
           "-compression_level", str(compression_level)]
    if vbr is not None:
        # LAME's own VBR scale: 0 is best, 9 worst. Beats CBR at equal size,
        # and -V0/-V2 are what people actually mean by "a good mp3".
        cmd += ["-q:a", str(vbr)]
    else:
        cmd += ["-b:a", bitrate]
    # ID3v2.3 plus a v1 tag: between them every player that exists reads the
    # metadata. -map_metadata 0 carries the source tags across.
    cmd += ["-map_metadata", "0", "-id3v2_version", "3", "-write_id3v1", "1",
            str(out)]
    return cmd
...
def do_transcode(ctx: Context, job: Job, tmpdir: Path) -> int:
    ...
    out = tmpdir / "out.mp3"
    r = run_with_retry(encode_cmd(url, out, ctx.bitrate, ctx.vbr,
    ...
    if ctx.art:
        cover = extract_embedded_cover(url, tmpdir) or ctx.folder_cover(job.rel)
        if cover:
            try:
...
def cmd_sync(args) -> int:
    ...
    def work(job: Job) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(dir=parent, prefix=".r2m-") as td:
                tmpdir = Path(td)
...
def check_one(ctx: Context, job: Job, tolerance: float) -> str | None:
    ...
    if st is None or st.get("codec_name") != "mp3":
        return f"not mp3 ({st.get('codec_name') if st else 'no audio stream'})"
    ...
    if src_info:
        ...
        if sd and dd and abs(sd - dd) > tolerance:
            return f"duration mismatch: source {sd:.2f}s vs mp3 {dd:.2f}s"
    return None
...
def build_parser(config: dict | None = None) -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="rsync2mp3",
        description="Mirror a lossless music library into MP3, incrementally.",
        epilog="Endpoints may be local paths or rsync-style [user@]host:/path.",
    )
    p.add_argument("--version", action="version", version=f"rsync2mp3 {__version__}")
    p.add_argument("--config", "-c", metavar="PATH",
                   help=f"config file (default: ./{CONFIG_NAME}, "
                        f"then ~/.config/rsync2mp3/config.toml)")
    ...
    endpoints(sp)
    sp.add_argument("--bitrate", "-b", default="256k",
                    help="constant target bitrate (default: %(default)s)")
    sp.add_argument("--vbr", "-V", type=int, metavar="0-9", default=None,
                    help="use LAME VBR at this quality instead of --bitrate "
                         "(0 best, 9 worst; 0 and 2 are the usual picks)")
    sp.add_argument("--compression-level", type=int, metavar="0-9", default=0,
                    help="encoder effort, 0 best to 9 fastest "
                         "(default: %(default)s — this is LAME's -q, search "
                         "effort, not bitrate)")
...
def main(argv: list[str] | None = None) -> int:
    for tool in ("ffmpeg", "ffprobe"):
        if shutil.which(tool) is None:
            print(f"rsync2mp3: {tool} not found in PATH", file=sys.stderr)
            return 2
    ...
    try:
        ...
        if vbr is not None and not 0 <= vbr <= 9:
            parser.error("--vbr takes a LAME quality from 0 (best) to 9 (worst)")
        level = getattr(args, "compression_level", 0)
        if not 0 <= level <= 9:
            parser.error("--compression-level takes 0 (best) to 9 (fastest)")
    except Fatal as e:
        print(f"rsync2mp3: {e}", file=sys.stderr)
        return 2

#!/usr/bin/env python3
"""
rsync2opus — mirror a lossless music library into Opus, incrementally.
...
Lossless sources (flac, wav, alac, ape, wv, ...) are transcoded to Opus.
Already-lossy sources (mp3, aac, opus, vorbis) are copied verbatim — they are
never re-encoded. Cover art, lyrics and playlists come along for the ride.
...
FAILURE_LOG = ".rsync2opus-failures.log"
TMP_SUFFIX = ".r2o-tmp"
    ...
    def _control_opts(cls) -> list[str]:
        with cls._control_lock:
            if cls._control_dir is None:
                ...
                cls._control_dir = tempfile.mkdtemp(prefix="r2opus-ssh-", dir=base)
    ...
    def listing(self) -> Listing:
        ...
        if r.returncode != 0:
            raise Fatal(
                ...
                f"{r.stderr.decode(errors='replace').strip()}\n"
                "(rsync2opus needs find, and stat on a BSD remote)"
            )
        out = Listing()
...
def plan(src_listing: dict[str, Entry], source: Endpoint,
    ...
    for rel, e in sorted(src_listing.items()):
        ...
        if ext in TRANSCODE_EXT:
            jobs.append(Job(rel, with_suffix(rel, ".opus"), "transcode", e.size, e.mtime))
    ...
    for rel, e in ambiguous:
        opus_rel = with_suffix(rel, ".opus")
        if not force and fresh(e.mtime, getattr(dst_listing.get(opus_rel), "mtime", None)):
            jobs.append(Job(rel, opus_rel, "transcode", e.size, e.mtime))
            continue
        ...
        jobs.append(Job(rel, opus_rel if lossless else rel,
...
def embed_cover(opus_path: Path, cover: tuple[bytes, str, int, int]) -> None:
    """Write cover art into an Ogg Opus file as METADATA_BLOCK_PICTURE.

    ffmpeg's Ogg muxer rejects attached picture streams outright, so art has
    to be added in a second pass.
    """
    from mutagen.flac import Picture
    from mutagen.oggopus import OggOpus

    data, mime, width, height = cover
    pic = Picture()
    pic.data = data
    pic.type = 3  # front cover
    pic.mime = sniff_mime(data, mime)
    pic.width, pic.height, pic.depth = width, height, 24

    f = OggOpus(str(opus_path))
...
class Context:
    ...
    def folder_cover(self, rel: str) -> tuple[bytes, str, int, int] | None:
        ...
        if cover_rel:
            try:
                with tempfile.TemporaryDirectory(prefix="r2o-art-") as td:
...
def parse_bitrate(spec: str) -> int:
    s = spec.strip().lower().rstrip("bps")
    mult = 1000 if s.endswith("k") else 1_000_000 if s.endswith("m") else 1
    return int(float(s.rstrip("km")) * mult)


# libopus caps out here; surround scaling must not sail past it.
OPUS_MAX_BITRATE = 510_000

# libopus only accepts a handful of channel layouts, and rejects common
# variants like 5.1(side) outright. This normalises them on the way in.
SURROUND_FILTER = "aformat=channel_layouts=7.1|5.1|quad|stereo|mono"


def encode_cmd(url: str, out: Path, bitrate: str, surround_channels: int = 0,
               vbr: str = "on", compression_level: int = 10) -> list[str]:
    cmd = ["ffmpeg", "-v", "error", "-y", "-i", url, "-map", "0:a:0", "-vn"]
    if surround_channels:
        # Multichannel Opus needs mapping family 1, and a bitrate that scales
        # with the channel count — stereo's budget spread over 6 channels
        # would sound thin.
        scaled = min(parse_bitrate(bitrate) * surround_channels // 2, OPUS_MAX_BITRATE)
        cmd += ["-af", SURROUND_FILTER, "-mapping_family", "1", "-b:a", str(scaled)]
    else:
        cmd += ["-b:a", bitrate]
    # vbr on is what makes --bitrate a target rather than a wall; off pins it to
    # a real CBR stream, for the rare player that insists on one. The
    # compression level is encoder effort only — 10 is slowest and best, and
    # lowering it buys speed at the cost of quality, not of file size.
    cmd += ["-c:a", "libopus", "-vbr", vbr,
            "-compression_level", str(compression_level),
            "-application", "audio", "-map_metadata", "0", str(out)]
    return cmd
...
def do_transcode(ctx: Context, job: Job, tmpdir: Path) -> int:
    ...
    out = tmpdir / "out.opus"
    r = run_with_retry(encode_cmd(url, out, ctx.bitrate, vbr=ctx.vbr,
                                  compression_level=ctx.compression_level))

    if r.returncode != 0 or not out.exists() or out.stat().st_size == 0:
        # Surround sources fail here rather than up front, so that the common
        # stereo path never pays for an extra probe.
        info = probe(url)
        st = audio_stream(info) if info else None
        channels = int(st.get("channels") or 0) if st else 0
        if channels > 2:
            r = run_with_retry(encode_cmd(url, out, ctx.bitrate,
...
def cmd_sync(args) -> int:
    ...
    ctx = Context(source, dest, args.bitrate, art=not args.no_art,
                  vbr=args.vbr, compression_level=args.compression_level,
    ...
    def work(job: Job) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(dir=parent, prefix=".r2o-") as td:
                tmpdir = Path(td)
...
def check_one(ctx: Context, job: Job, tolerance: float) -> str | None:
    ...
    if st is None or st.get("codec_name") != "opus":
        return f"not opus ({st.get('codec_name') if st else 'no audio stream'})"
    ...
    if src_info:
        ...
        if sd and dd and abs(sd - dd) > tolerance:
            return f"duration mismatch: source {sd:.2f}s vs opus {dd:.2f}s"
    return None
...
def cmd_verify(args) -> int:
    ...
    ctx = Context(source, dest, "128k", art=False)
...
def build_parser(config: dict | None = None) -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="rsync2opus",
        description="Mirror a lossless music library into Opus, incrementally.",
    ...
    endpoints(sp)
    sp.add_argument("--bitrate", "-b", default="128k",
                    help="opus target bitrate (default: %(default)s)")
    sp.add_argument("--vbr", choices=("on", "constrained", "off"), default="on",
                    help="bitrate mode (default: %(default)s; `off` gives a "
                         "true CBR stream for players that need one)")
    sp.add_argument("--compression-level", type=int, metavar="0-10", default=10,
                    help="encoder effort, 0 fastest to 10 best "
                         "(default: %(default)s — lowering it trades quality "
                         "for speed, not size)")
...
def main(argv: list[str] | None = None) -> int:
    for tool in ("ffmpeg", "ffprobe"):
        if shutil.which(tool) is None:
            print(f"rsync2opus: {tool} not found in PATH", file=sys.stderr)
            return 2
    ...
    try:
        ...
        level = getattr(args, "compression_level", 10)
        if not 0 <= level <= 10:
            parser.error("--compression-level takes 0 (fastest) to 10 (best)")
    except Fatal as e:
        print(f"rsync2opus: {e}", file=sys.stderr)
        return 2
